...
def make_region_actors_bubble(missions_df):
    ...
    fig = px.scatter(
        ...
        color="Actors",
        text="Actors",
        hover_data={"Missions": True, "Activities": True, "Records": True},
...
def make_region_activities_bubble(missions_df):
    ...
    fig = px.scatter(
        ...
        color="Activities",
        text="Activities",
        hover_data={"Missions": True, "Actors": True, "Records": True},
...
def make_avg_records_region_chart(summary):
    df = summary.sort_values("Avg_Records_Per_Mission", ascending=True)

    fig = px.bar(
        df,
        x="Avg_Records_Per_Mission",
        y="regions",
        orientation="h",
        text="Avg_Records_Per_Mission",
        color="Avg_Records_Per_Mission",
        color_continuous_scale="Blues",
        title="Average Records per Mission by Region",
        hover_data={
            "Mission_Reports": True,
            "Total_Records": True,
            "Avg_Records_Per_Mission": ":.2f",
            "regions": False
        }
    )
...
def render_region_comparison(
    ...
    with c1:
        ...
        fig = make_avg_records_region_chart(summary)
        st.plotly_chart(fig, use_container_width=True, key="regional_avg_records")
    ...
    with c6:
        ...
        record_type_df = (
            records
            .groupby(["regions", "report_record_type"], dropna=False)
            .size()
            .reset_index(name="count")
        )

        if not record_type_df.empty:
            fig = px.bar(
                record_type_df,
                x="count",
            ...
            st.plotly_chart(fig, use_container_width=True, key="regional_record_types")
        else:
            st.info("No record-type data available for the current filters.")
...
def render_reports_dashboard():
    ...
    with overview_tab:
        ...
        with c5:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.plotly_chart(make_region_actors_bubble(filtered_missions), use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)

        with c6:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.plotly_chart(make_region_activities_bubble(filtered_missions), use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
